...
template <typename T, int ALIGN, int PAD1>
...
class TaggedValue {
   public:
    ...
    explicit _always_inline TaggedValue(T value, tag_type tag) {
        ...
        if ((this->value() != value) || (this->tag() != tag)) {
            fprintf(stderr,
                    "TaggedValue inconsistency: tried creating with "
                    "value: %p, tag: %d. result: value(): %p, tag(): %d.\n",
                    (void*)value, tag, (void*)this->value(),
                    this->tag());  // NOLINT
            abort();
        }
#endif                                // TAGGED_VALUE_CHECKED_MODE
   ...
   private:
    ...
    template <typename S, int ALIGN, int PAD1>
    friend class AtomicTaggedValue;
};

template <typename T, int ALIGN = 0, int PAD1 = 0>
class AtomicTaggedValue {
   public:
    _always_inline AtomicTaggedValue() : raw_atomic_(0) {}

    explicit _always_inline AtomicTaggedValue(
        const TaggedValue<T>& tagged_value)
    ...
    _always_inline void operator delete(void* ptr) { free(ptr); }

   private:
    std::atomic<typename TaggedValue<T>::raw_type> raw_atomic_;
    uint8_t _pad1ding
        [(PAD1 != 0)
             ? PAD1 - sizeof(std::atomic<typename TaggedValue<T>::raw_type>)
             : 0];
};
