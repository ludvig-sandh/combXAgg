...
class Settings:
    ...
    minDuration_ms = 0.1    ## min duration required for an interval to be rendered
    ...
    cmsequence=cm.bone(np.linspace(0, .66, numColors))  ## numColors colors picked linearly from the specified colormap
    ...
    def __init__(self, infile, outfile, title, subtitle, pairs):
        self.infile = infile
...
if len(sys.argv) < 4:
    print('USAGE: python timeline_advplot.py <infile> <outfile> <title> <subtitle> <event> <color> [[<event> <color>] ...]')
    print()
    ...
    print()
    print('       <title> should be a string title (possibly empty) for the plot')
    print()
    print('       <subtitle> should be a string subtitle (possibly empty) for the plot')
    print()
...
print(sys.argv)
settings = Settings(infile=sys.argv[1], outfile=sys.argv[2], title=sys.argv[3], subtitle=sys.argv[4], pairs=sys.argv[5:])
...
for line in fileinput.input(settings.infile):
    ...
    if settings.includeLine(line) == True:
        ...
        if duration >= 24*3600*1000*scale: ## interval longer than a year...
            print()
            print('ERROR: an interval is longer than a day. this is obviously wrong.')
            print('    offending line:')
            print('    {}'.format(line))
            print('    if this interval came from setbench/tmbench,')
            print('    you might have simply hit the capacity of the GSTATS_ variable')
            print('    that is storing your timeline/blip events...')
            exit(1)
...
## set plot title if applicable
if settings.title:
    plt.suptitle(settings.title, fontsize=64)
if settings.subtitle:
    plt.title(settings.subtitle, fontsize=64)
...
ax.tick_params(axis='both', which='minor', labelsize=64)


## add x-axis title
ax.xaxis.label.set_visible(True)
ax.set_xlabel('time in nanoseconds', fontsize=64)

## add y-axis title
ax.yaxis.label.set_visible(True)
ax.set_ylabel('thread number', fontsize=64)
