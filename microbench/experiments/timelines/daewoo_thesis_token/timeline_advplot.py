...
class Settings:
    ...
    def __init__(self, infile, outfile, title, pairs):
        self.infile = infile
...
if len(sys.argv) < 4:
    print('USAGE: python timeline_advplot.py <infile> <outfile> <title> <event> <color> [[<event> <color>] ...]')
    print()
...
print(sys.argv)
settings = Settings(infile=sys.argv[1], outfile=sys.argv[2], title=sys.argv[3], pairs=sys.argv[4:])
...
if settings.title:
    fig.suptitle(settings.title, fontsize=16)
