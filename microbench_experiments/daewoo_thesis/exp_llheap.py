...
def define_experiment(exp_dict, args):
    ...
    set_dir_run     ( exp_dict, os.getcwd() + '/bin_llheap' )         ## working directory where your program should be executed
    set_dir_data    ( exp_dict, os.getcwd() + '/data_llheap' )        ## directory for data files produced by individual runs of your program
    ...
    add_run_param     ( exp_dict, 'alloc'           , ['llheap', 'jemalloc', 'mimalloc', 'tcmalloc', 'hoard', 'glibc', 'super0', 'super3'] )
    ...
    add_run_param     ( exp_dict, 'MILLIS_TO_RUN'   , [5000, 60000])
    ...
    if args.testing:
        ...
        set_cmd_run     ( exp_dict, 'LD_PRELOAD=../../../lib/lib{alloc}.so timeout 300 numactl --interleave=all time ./{DS_TYPENAME}.debra -nwork {TOTAL_THREADS} -nprefill {TOTAL_THREADS} -insdel {INS_DEL_FRAC} -k {MAXKEY} -t {MILLIS_TO_RUN} {thread_pinning} -rq 0 -rqsize 1 -nrq 0' )
    ...
    plot_cmd_args = '--legend-include --legend-columns 4 --lightmode --error-bar-width 4'
    ...
    add_plot_set(
            exp_dict
          , name='throughput-allocs-{MILLIS_TO_RUN}.png'
    ...
    add_plot_set(
            exp_dict
          , name='maxresident-allocs-{MILLIS_TO_RUN}.png'
          , varying_cols_list=['MILLIS_TO_RUN']
