...
def define_experiment(exp_dict, args):
    ...
    set_dir_data    ( exp_dict, os.getcwd() + '/data_llheap_240amortized' )   ## directory for data files produced by individual runs of your program
    ...
    add_run_param     ( exp_dict, 'alloc'           , ['super0', 'super3', 'jemalloc', 'tcmalloc', 'hoard', 'mimalloc', 'glibc', 'llheap'] )
    # add_run_param     ( exp_dict, 'alloc'           , ['supermalloc', 'super1', 'super2', 'super3', 'super4', 'super5'] )
    # add_run_param     ( exp_dict, 'alloc'           , ['supermalloc', 'jemalloc', 'tcmalloc', 'hoard'] )
    add_run_param     ( exp_dict, 'INS_DEL_FRAC'    , ['50.0 50.0'] )
    add_run_param     ( exp_dict, 'MAXKEY'          , [20000000] )
    add_run_param     ( exp_dict, 'DS_TYPENAME'     , ['brown_ext_abtree_lf'] )
    add_run_param     ( exp_dict, 'MILLIS_TO_RUN'   , [1000, 2000, 4000, 8000, 16000, 32000, 64000])
    ...
    add_run_param     ( exp_dict, 'TOTAL_THREADS'   , [240] ) #shell_to_listi('cd ' + get_dir_tools(exp_dict) + ' ; ./get_thread_counts_max.sh', exit_on_error=True) )
    ...
    set_cmd_compile     ( exp_dict, 'make bin_dir={__dir_run} -j debra_orig_free=0' )
    ...
    plot_cmd_args = '--legend-include --legend-columns 4 --error-bar-width 4'
    ...
    filters = [
      ...
      # , 'alloc in ("super0", "super3")',
      # , 'alloc in ("super0", "super3", "jemalloc", "tcmalloc", "glibc", "llheap")',
      # , 'alloc in ("super0", "super3", "jemalloc", "tcmalloc", "hoard", "mimalloc", "glibc", "llheap")'
    ]

    for group, filter_str in zip(range(len(filters)), filters):
      add_plot_set(
              exp_dict
            , name='group{}-throughput-vs-time-noprefill.png'.format(group)
            , title='Throughput vs time'
            , series='alloc'
            , filter=filter_str
            , x_axis='MILLIS_TO_RUN'
      ...
      add_plot_set(
              exp_dict
            , name='group{}-maxresident-vs-time-noprefill.png'.format(group)
            , title='Max resident size (MB) vs time'
            , series='alloc'
            , filter=filter_str
            , x_axis='MILLIS_TO_RUN'
