...
# filtered = data.query('(INS == {} and MAXKEY == {})'.format(ins, k))
...
# tmean = pd.pivot_table(filtered, index='TOTAL_THREADS', values='PAPI_L3_TCM', columns='DS_TYPENAME', aggfunc='mean')
# tmin = pd.pivot_table(filtered, index='TOTAL_THREADS', values='PAPI_L3_TCM', columns='DS_TYPENAME', aggfunc='min')
# tmax = pd.pivot_table(filtered, index='TOTAL_THREADS', values='PAPI_L3_TCM', columns='DS_TYPENAME', aggfunc='max')
...
# print("EXTRACTING e in err[0]")
# for e in err[0]:
#     print("x in e")
#     print([x for x in e.index])
# # print([e for e in err[0]])
...
fig, ax = plt.subplots()
if args.error_bar_width == 0:
    chart = tmean.plot(ax=ax, legend=False, title=args.title, kind='bar', figsize=(args.width_inches, args.height_inches), width=0.75, edgecolor='black', linewidth=3, zorder=10, logy=args.log_y)
else:
    ...
    chart = tmean.plot(ax=ax, legend=False, title=args.title, kind='bar', yerr=err, error_kw=dict(elinewidth=args.error_bar_width, ecolor='red'), figsize=(args.width_inches, args.height_inches), width=0.75, edgecolor='black', linewidth=3, zorder=10, logy=args.log_y)
...
if args.legend_include:
    # plt.legend(title=None) #, bbox_to_anchor=(1.04,0.5), loc="center left", borderaxespad=0)
...
if args.legend_only:
    ...
    fig_legend.savefig(args.outfile.name)
