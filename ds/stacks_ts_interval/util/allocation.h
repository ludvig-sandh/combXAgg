...
ThreadLocalAllocator& ThreadLocalAllocator::Get() {
    ...
    ThreadLocalAllocator* tla =
        static_cast<ThreadLocalAllocator*>(pthread_getspecific(tla_key));
    if (tla == NULL) {
...
void* ThreadLocalAllocator::Malloc(size_t size) {
    ...
    if (size > prealloc_size_) {
        fprintf(stderr,
                "unable to allocate %lu bytes through "
...
void* ThreadLocalAllocator::CallocAligned(size_t num, size_t size,
                                          size_t alignment) {
...
size_t HumanSizeToPages(const char* hsize, size_t len) {
    ...
    if (val == 0) {
        fprintf(stderr,
                "%s: strtol() returned 0; do not try to set the "
                "prealloc_size to 0\n",
                __func__);
        abort();
    }
    const size_t bytes = val * multiplier;
    return ((bytes % kPageSize) == 0) ? bytes / kPageSize
                                      : bytes / kPageSize + 1;
}
