    ...
    explicit KSegment(uint64_t k)
        : remove(0),
          ...
          items(static_cast<AtomicItem*>(
              ThreadLocalAllocator::Get().CallocAligned(k, sizeof(*items),
                                                        64))) {
#ifdef LOCALLY_LINEARIZABLE
...
class KStack : public Stack<T> {
   ...
   private:
    ...
    inline bool find_index(KSegment* segment, bool empty, uint64_t* item_index,
                           TaggedValue<T>* old);
    bool try_add_new_ksegment(const TaggedValue<KSegment*>& top_old,
                              const T& item);
    void try_remove_ksegment(const TaggedValue<KSegment*>& top_old);
    bool committed(TaggedValue<KSegment*> top_old,
                   const TaggedValue<T>& item_new, uint64_t index);
...
KStack<T>::KStack(uint64_t k, uint64_t num_threads)
    : top_(new AtomicTopPtr(SegmentPtr(new KSegment(k), 0))), k_(k) {}
...
template <typename T>
bool KStack<T>::try_add_new_ksegment(const TaggedValue<KSegment*>& top_old,
                                     const T& item) {
...
template <typename T>
void KStack<T>::try_remove_ksegment(const TaggedValue<KSegment*>& top_old) {
    SegmentPtr next = top_->load().value()->next.load();
    if (top_->load() == top_old) {
        if (next.value() != NULL) {
            __sync_fetch_and_add(&top_old.value()->remove, 1);
            if (is_empty(top_old.value())) {
                if (top_->swap(top_old,
                               SegmentPtr(next.value(), top_old.tag() + 1))) {
                    return;
                }
            }
...
template <typename T>
bool KStack<T>::committed(TaggedValue<KSegment*> top_old,
                          const TaggedValue<T>& item_new, uint64_t index) {
    if (top_old.value()->items[index].load() != item_new) {
        ...
        if (top_->load() != top_old) {
            ...
            if (top_->swap(top_old,
                           SegmentPtr(top_old.value(), top_old.tag() + 1))) {
                return true;
            }
            if (!top_old.value()->items[index].swap(
                    item_new, Item((T)NULL, item_new.tag() + 1))) {
...
template <typename T>
bool KStack<T>::find_index(KSegment* segment, bool empty, uint64_t* item_index,
                           TaggedValue<T>* old) {
    ...
    while (true) {
        ...
        if (top_->load() == top_old) {
            if (found_idx) {
                ...
                if (top_old.value()->items[item_index].swap(item_old,
                                                            item_new)) {
    ...
    while (true) {
        ...
        if (top_->load() == top_old) {
            if (found_idx) {
                ...
                if (top_old.value()->next.load().value() ==
                    NULL) {  // is last segment
                    if (is_empty(top_old.value())) {
                        if (top_->load() == top_old) {
                            return false;
                        }
