...
struct alignas(PREFETCH_SIZE_BYTES) Batch {
    // PAD
    std::atomic<int> *eliminationArray;
    PAD;
   ...
   private:
    ...
    struct Batch<K, V> *CreateNewBatch() {
        ...
        newBatch->eliminationArray =
            malloc(sizeof(std::atomic<int>) * MAX_AGGREGATOR_THREADS);
        for (size_t i = 0; i < MAX_AGGREGATOR_THREADS; i++) {
            newBatch->eliminationArray[i].store(-1);
        }
        return newBatch;
    }
    ...
    void CreatePushSubstackAndPush(struct Batch<K, V> *batch, int leaderIndex) {
        int num_push = batch->finalPushCount - batch->finalPopCount;
        nodeptr block = malloc(sizeof(node_t<K, V>) * num_push);
        block[0].val = batch->eliminationArray[leaderIndex].load();
        for (size_t i = 1; i < num_push; i++) {
            block[i - 1].next = &block[i];
            while (batch->eliminationArray[leaderIndex + i].load() < 0) {
            }
            block[i].val = batch->eliminationArray[leaderIndex + i].load();
        }
        while (true) {
            struct node_t<K, V> *top = main_top;
            block[num_push - 1].next = top;
            if (main_top.compare_exchange_strong(top, &block[0])) return;
        }
    }
    ...
    bool push(const int &tid, const V &value) {
        ...
        while (true) {
            ...
            int pushIndex = myBatch->pushCounter.fetch_add(
                1);  // Opt. Check software F&A speed up? //FIXMEURGENT: fetch
                     // ad shoould just take 1 int param other should be
                     // memoryorder.
            myBatch->eliminationArray[pushIndex].store(tid);
            ...
            if (pushIndex <
                myBatch->finalPopCount.load(     // Spin until freezing has
                    std::memory_order_acquire))  // Eliminated.
            {
                // COUTATOMICTID("dummy eliminated " << value << std::endl);
                if ((myBatch->finalPushCount.load(std::memory_order_acquire) ==
    ...
    bool pop(const int &tid) {
        ...
        while (true) {
                               ...
                               std::memory_order_acquire))  // Eliminated.
            {
                ...
                V returnValue = myBatch->eliminationArray[popIndex].load();
