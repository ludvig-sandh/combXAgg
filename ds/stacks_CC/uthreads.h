...
/// @brief This file exposes the API by the Synch framework for creating fibers
/// inside a posix thread.
...
/// @param max The maximum number of fibers that the current posix thread could
/// create.
void synchInitFibers(int max);

/// @brief This function gives the control of the processor to the next fiber of
/// the current posix thread.
void synchFiberYield(void);

/// @brief This function spawns a new fiber inside the current posix thread.
/// @param func A pointer to a function that newely spawned fiber will execute
/// after its creation.
/// @param arg An argument passed to the newely created thread.
int synchSpawnFiber(void* (*func)(void*), long arg);

/// @brief Wait for all of the fibers that are running inside the current posix
/// thread to finish their execution.
void synchWaitForAllFibers(void);

/// @brief This function returns an index of the running fiber in the current
/// posix thread, i.e. an integer number between 0 and N-1, where N is the
/// number of active fibers in the current posix thread.
...
void synchInitFibers(int max) {
    ...
    MAX_FIBERS = max;
    FIBER_LIST =
        reinterpret_cast<Fiber*>(synchGetMemory(MAX_FIBERS * sizeof(Fiber)));
    getcontext(&(FIBER_LIST[0].context));
    FIBER_LIST[0].active = true;
    FIBER_LIST[0].context.uc_link = NULL;
    for (i = 1; i < MAX_FIBERS; i++) FIBER_LIST[i].active = false;
}
...
static void fiber_start_func(FiberData* context) {
    ...
    func = context->func;
    arg = context->arg;
    if (_setjmp(*(context->cur)) == 0) swapcontext(&tmp, context->prev);
    ...
    FIBER_RECYCLE = reinterpret_cast<Fiber*>(
        FIBER_LIST[currentFiber].context.uc_stack.ss_sp);
...
int synchSpawnFiber(void* (*func)(void*), long arg) {
    ...
    context.prev = &tmp;
    makecontext(&FIBER_LIST[N_FIBERS].context, (void (*)())fiber_start_func, 1,
                &context);
...
void synchWaitForAllFibers(void) {  // Execute the fibers until they quit
    while (N_FIBERS > 1) synchFiberYield();
}

int32_t synchCurrentFiberIndex(void) { return currentFiber; }
