...
/// @brief This file exposes a simple API for implementing a very simple pool
/// object. This pool object gives user the ability to allocate small chunks of
/// memory (e.g. allocating nodes for using them in an queue or stack
/// implementation) in a fast and efficient way. The main purpose of this pool
/// implementation is to add minimal overheads while benchmarking concurrent
/// data structures, such as stacks. queues, etc. This object does not provide
/// thread-safe methods for accessing, and thus each of the running threads
/// should use its own instance without directly accessing the pool of any other
/// thread.
#ifndef _POOL_H_
#define _POOL_H_
...
typedef struct SynchBlockObject {
    /// @brief The first field of a block object is a pointer to the next
    /// allocated block (if any).
...
typedef struct SynchPoolBlockMetadata {
    ...
    /// @brief The first free block. This should be returned in the next call of
    /// synchAllocObj.
    uint32_t cur_entry;
...
/// @brief This struct stores the metadata of the block and all the objects of
/// the block.
typedef struct SynchPoolBlock {
    /// @brief The metadata of the block.
    SynchPoolBlockMetadata metadata;
    /// @brief The actual storage space of the block (i.e. the objects of the
    /// block).
    char heap[];
} SynchPoolBlock;
...
typedef struct SynchPoolStruct {
    ...
    /// @brief The head of the list of blocks, where each block stores a
    /// specific amount of objects.
...
/// @brief This is returned in case of error while calling synchAllocObj
/// (usually system's memory is exhausted).
...
/// @return In case of success, synchInitPool returns SYNCH_POOL_INIT_SUCC. In
/// case of error, synchInitPool returns SYNCH_POOL_INIT_ERROR.
int synchInitPool(SynchPoolStruct* pool, uint32_t obj_size);

/// @brief This function initializes a pool with objects of size obj_size.
/// @param pool A pointer to the pool of objects.
/// @return On success, a pointer to a free object is returned. Otherwise,
/// SYNCH_POOL_OBJECT_ALLOC_ERROR is returned.
...
/// @brief This function cancels the last num_objs consecutive object
/// allocations. Note that no recycle_obj operation should have been called for
/// any of the last num_objs consecutive object allocations.
/// @param pool A pointer to the pool of objects.
/// @param num_objs The number of consecutive allocations that should be
/// canceled.
...
static void* get_new_block(uint32_t obj_size) {
    ...
    block = reinterpret_cast<SynchPoolBlock*>(
        synchGetAlignedMemory(CACHE_LINE_SIZE, BLOCK_SIZE_CC));
    block->metadata.entries =
        (BLOCK_SIZE_CC - POOL_BLOCK_METADATA_SIZE) / obj_size;
    block->metadata.free_entries =
        (BLOCK_SIZE_CC - POOL_BLOCK_METADATA_SIZE) / obj_size;
...
int synchInitPool(SynchPoolStruct* pool, uint32_t obj_size) {
    ...
    block = reinterpret_cast<SynchPoolBlock*>(get_new_block(obj_size));
...
void* synchAllocObj(SynchPoolStruct* pool) {
    ...
    if (pool->recycle_list == NULL) {
        if (pool->cur_block->metadata.free_entries > 0) {
            ret = reinterpret_cast<SynchBlockObject*>(
                (void*)&pool->cur_block
                    ->heap[(pool->cur_block->metadata.cur_entry) *
                           (pool->obj_size)]);
            ...
            if (pool->cur_block->metadata.next != NULL) {
                ...
                SynchPoolBlock* new_block = reinterpret_cast<SynchPoolBlock*>(
                    get_new_block(pool->obj_size));
                new_block->metadata.back = pool->cur_block;
                pool->cur_block = new_block;
#ifdef DEBUG_POOL_STATS
                COUTATOMIC("allocating new"
                           << ++recrcntr << ":" << num_avoided_new
                           << " objsz:" << pool->obj_size << std::endl);
#endif
            }
            ret = reinterpret_cast<SynchBlockObject*>(synchAllocObj(pool));
        }
    } else {
...
#ifdef DEBUG
    if (ret == NULL)
        fprintf(stderr, "DEBUG: synchAllocObj returns a NULL object\n");
#endif
...
void synchRecycleObj(SynchPoolStruct* pool, void* obj) {
    ...
    SynchBlockObject* object = reinterpret_cast<SynchBlockObject*>(obj);
...
void synchRollback(SynchPoolStruct* pool, uint32_t num_objs) {
    while (num_objs > 0) {
        if (num_objs > pool->cur_block->metadata.cur_entry) {
            ...
            pool->cur_block->metadata.free_entries =
                pool->cur_block->metadata.entries;
