/// @file primitives.h
/// @brief This file exposes a simple API for basic atomic operations, basic
/// operations for memory management. Moreover, this file provides functionality
/// for finding out the the vendor of the processor and some basic functionality
/// for measuring time.
...
#define synchStoreFence() asm volatile("sfence" ::: "memory")
#define synchFullFence() asm volatile("mfence" ::: "memory")
#define synchReadPrefetch(A) \
    asm volatile("prefetchnta %0" ::"m"(*((const int *)A)))
#define synchStorePrefetch(A) \
    asm volatile("prefetchnta %0" ::"m"(*((const int *)A)))
#define synchNonTSOFence()
#define synchLikely(A) (A)
#define synchUnlikely(A) (A)
#define UNUSED_ARG __attribute__((unused))
//   in this case where gcc is too old, implement atomic primitives in
//   primitives.c
...
/// @brief This macro is designed to emit a pause instruction specifically for
/// Intel X86 processors. The inclusion of a pause instruction in spinning loops
/// can significantly improve performance on Intel X86 machines by reducing the
/// execution resource requirements and improving the performance in many SMT
/// scenarios. It effectively hints to the processor that it is in a spin-wait
/// loop, allowing the CPU to handle the other thread more efficiently or reduce
/// power consumption.
///
/// Note: This optimization is particularly beneficial on Intel X86
/// architectures and does not yield the same performance benefits on AMD
/// processors.
#define synchPause()                                             \
    {                                                            \
        int __i;                                                 \
...
/// In case that SYNCH_NUMA_SUPPORT_FC is defined in libconcurrent/config.h, the
/// returned memory is allocated on the local NUMA node.
///
/// @param size The size of the memory area.
/// @return In case of error, NULL is returned. In case of success a pointer to
/// the allocated memory area is returned.
inline void *synchGetMemory(size_t size);

/// @brief This function allocates a memory area of size bytes. The returned
/// address is aligned to an offset equal to align bytes. In case that
/// SYNCH_NUMA_SUPPORT_FC is defined in libconcurrent/config.h, the returned
...
/// @brief This function frees memory allocated with either getMemory() or
/// synchGetAlignedMemory() functions.
///
...
/// The current version of the Synch framework returns any of the following
/// codes:
/// - AMD_X86_MACHINE
/// - INTEL_X86_MACHINE
/// - X86_GENERIC_MACHINE
...
#define synchCAS128(A, B0, B1, C0, C1)                                       \
    _CAS128((uint64_t *)(A), (uint64_t)(B0), (uint64_t)(B1), (uint64_t)(C0), \
            (uint64_t)(C1))
/// @brief This function is executed atomically. It reads the 128-bit value
/// stored at a memory location pointed by A. Let A0 be the less significant
/// 64-bits of this memory location and let A1 the most significant. This
/// function computes '(A0 == B0 & A1 == B1) ? <C0,C1> : <A0,A1>' and stores the
/// result at location pointed by A. The function returns in case that (A0 == B0
/// & A1 == B1) is true. Otherwise, it returns false.
///
...
/// @return This function computes '(A0 == B0 & A1 == B1) ? <C0,C1> : <A0,A1>'
/// and stores the result at location pointed by A. The function returns in case
/// that (A0 == B0 & A1 == B1) is true. Otherwise, it returns false.
...
/// @brief This function is executed atomically. It reads the pointer stored at
/// a memory location pointed by A. Let OLD be the pointer of the memory
/// location pointed by A. This function computes '(OLD == B) ? C : OLD' and
/// stores the result at location pointed by A. The function returns in case
/// that (OLD == B) is true. Otherwise, it returns false.
///
/// @param A A pointer to memory location that stores a memory pointer.
/// @param B The old value.
/// @param C The new value.
/// @return The function returns in case that (OLD == B) is true. Otherwise, it
/// returns false.
inline bool _CASPTR(void *A, void *B, void *C);

/// A wrapper for the _CAS64 function. See more on _CAS64().
#define synchCAS64(A, B, C) \
    _CAS64((uint64_t *)(A), (uint64_t)(B), (uint64_t)(C))
/// @brief This function is executed atomically. It reads the 64-bit value
/// stored at a memory location pointed by A. Let OLD be the value of the memory
/// location pointed by A. This function computes '(OLD == B) ? C : OLD' and
/// stores the result at location pointed by A. The function returns in case
/// that (OLD == B) is true. Otherwise, it returns false.
///
...
#define synchCAS32(A, B, C) \
    _CAS32((uint32_t *)(A), (uint32_t)(B), (uint32_t)(C))
/// @brief This function is executed atomically. It reads the 32-bit value
/// stored at a memory location pointed by A. Let OLD be the value of the memory
/// location pointed by A. This function computes '(OLD == B) ? C : OLD' and
/// stores the result at location pointed by A. The function returns in case
/// that (OLD == B) is true. Otherwise, it returns false.
...
/// @brief This function is executed atomically. It performs an atomic exchange
/// operation on the memory location pointed by A, setting B as the new value.
/// It returns the old value of the memory location pointed by A just before the
/// operation.
///
/// @param A A pointer to memory location that stores a memory pointer.
/// @param B The new value to be stored in the memory location pointed by A.
/// @return It returns the old value of the memory location pointed by A just
...
/// @brief This function is executed atomically. It performs an atomic addition
/// of value B to the value pointed by A. It returns the old value of the memory
/// location pointed by A just before the operation.
///
...
/// @return It returns the old 32-bit value of the memory location pointed by A
...
/// @return It returns the old 64-bit value of the memory location pointed by A
/// just before the operation.
inline int64_t _FAA64(volatile int64_t *A, int64_t B);

/// A wrapper for the _BitTAS64 function. See more on _BitTAS64().
#define synchBitTAS64(A, B) \
    _BitTAS64((volatile uint64_t *)(A), (unsigned char)(B))
/// @brief This function is executed atomically. It performs an atomic Test&Set
/// on the B-th bit of the value pointed by A.
///
/// @param A A pointer to memory location that stores a 64-bit value.
/// @param B The B-th bit of the value stored in the memory location pointed by
/// A.
