...
static const uint32_t BLOCK_SIZE_CC = 4096;

static void *get_new_block(uint32_t obj_size) {
    SynchPoolBlock *block;
    block =
        reinterpret_cast <
        SynchPoolBlock *> (synchGetAlignedMemory(CACHE_LINE_SIZE, BLOCK_SIZE_CC));
...
void *synchAllocObj(SynchPoolStruct *pool) {
    ...
    if (pool->recycle_list == NULL) {
        if (pool->cur_block->metadata.free_entries > 0) {
            ret = reinterpret_cast<SynchBlockObject*>((void*) &
                  pool->cur_block->heap[(pool->cur_block->metadata.cur_entry) *
                                        (pool->obj_size)]);
            ...
            if (pool->cur_block->metadata.next != NULL) {
                ...
                SynchPoolBlock* new_block =
                    reinterpret_cast<SynchPoolBlock*> (get_new_block(
                        pool->obj_size));
    ...
    if (ret == NULL) fprintf(stderr, "DEBUG_FC_STACK: synchAllocObj returns a NULL object\n");
#endif
