...
int synchThreadPin(int32_t cpu_id) {
    ...
    if (__preferred_core % 2 == 0) {
        ...
        __preferred_core = ((__preferred_core - 1) / 2) + (synchGetNCores() / 4);
    }
    ...
    core = ((core - (core % numa_node_count)) / numa_node_count) + ((core % numa_node_count)*(core_count / virtual_cores_per_core / numa_node_count));
