...
/// An example of use of this API is provided in benchmarks/dsmsynchbench.c
/// file.
///
...
/// @brief DSMSynchThreadState stores each thread's local state for a single
/// instance of DSMSynch. For each instance of DSMSynch, a discrete instance of
/// DSMSynchThreadState should be used.
...
/// @brief DSMSynchStruct stores the state of an instance of the a DSMSynch
/// combining object. DSMSynchStruct should be initialized using the
/// DSMSynchStructInit function.
typedef struct DSMSynchStruct {
    ...
    /// @brief The number of threads that will use the DSMSynch combining
    /// object.
    uint32_t nthreads;
#ifdef DEBUG
...
/// @brief This function initializes an instance of the DSMSynch combining
/// object.
///
/// This function should be called once (by a single thread) before any other
/// thread tries to apply any request by using the DSMSynchApplyOp function.
///
/// @param l A pointer to an instance of the DSMSynch combining object.
/// @param nthreads The number of threads that will use the DSMSynch combining
/// object.
...
/// operation to the DSMSynch combining object.
///
/// @param l A pointer to an instance of the DSMSynch combining object.
/// @param st_thread A pointer to thread's local state of DSMSynch.
/// @param pid The pid of the calling thread.
void DSMSynchThreadStateInit(DSMSynchStruct *l, DSMSynchThreadState *st_thread,
                             int pid);
...
/// of DSMSynch.
/// @param sfunc A serial function that the DSMSynch instance should execute,
...
RetVal DSMSynchApplyOp(DSMSynchStruct *l, DSMSynchThreadState *st_thread,
...
RetVal DSMSynchApplyOp(DSMSynchStruct *l, DSMSynchThreadState *st_thread,
    ...
    if (p->next == NULL) {
        if (l->Tail == p && synchCASPTR(&l->Tail, p, NULL) == true)
            return mynode->arg_ret;
        while (p->next == NULL) {
            synchResched();
        }
...
void DSMSynchStructInit(DSMSynchStruct *l, uint32_t nthreads) {
    ...
    if (synchGetMachineModel() == INTEL_X86_MACHINE) {
        ...
        l->nodes = synchGetAlignedMemory(CACHE_LINE_SIZE,
                                         2 * nthreads * sizeof(DSMSynchNode));
    }
...
void DSMSynchThreadStateInit(DSMSynchStruct *l, DSMSynchThreadState *st_thread,
    ...
    if (synchGetMachineModel() == INTEL_X86_MACHINE) {
        DSMSynchNode *nodes =
            synchGetAlignedMemory(CACHE_LINE_SIZE, 2 * sizeof(DSMSynchNode));
